...
//             // lat: updatedPost.lat || null, // Gunakan data latitude jika tersedia
//             // lon: updatedPost.lon || null, // Gunakan data longitude jika tersedia
//         };
...
const updatePost = (req, res) => {
    ...
    Post.findByIdAndUsername(id, username, (err, post) => {
        ...
        // Jika post ditemukan, lakukan update
        Post.update(id, description, (err, updatedPost) => {
            if (err) {
                return res.status(500).json({
                    error: true,
