...
//             await bucketProfile}.upload(imageFile.path, {
...
//                 bmi: bmi || null,
//                 health_condition: health_condition || null,
...
// // Gunakan memoryStorage untuk menyimpan file langsung di memori
// const storage = multer.memoryStorage();
...
//         const imageFile = req.file;  // Mengambil file yang diunggah
//         const userId = uuidv4();
//         const hashedPassword = bcrypt.hashSync(password, 8);
...
//         // Membuat nama file dengan timestamp dan ekstensi asli
//         const fileName = Date.now() + path.extname(imageFile.originalname);

//         try {
//             // Upload file ke GCS menggunakan buffer dari memory
...
//                     await User.create(newUser);  // Menggunakan async/await untuk menangani pembuatan user
//                     res.status(201).json({
//                         error: false,
//                         message: 'User registered successfully',
//                     });
...
//             // Tulis data file ke stream
//             stream.end(imageFile.buffer); // Menulis buffer ke GCS
...
// const updateUser = (req, res) => {
//     const { name, username, email, password, weight, height, blood_sugar, blood_pressure, bmi } = req.body;
//     const userId = req.user.id; 
